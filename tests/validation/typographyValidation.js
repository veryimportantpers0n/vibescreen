...
class TypographyValidator {
  ...
  async validateTypographyImplementation() {
    ...
    const globalsCSS = fs.readFileSync(path.join(path.dirname(__dirname), '..', 'styles/globals.css'), 'utf8');
    this.test(
    ...
    const terminalCSS = fs.readFileSync(path.join(path.dirname(__dirname), '..', 'styles/terminal-interface.module.css'), 'utf8');
    this.test(
    ...
    const modeThemesCSS = fs.readFileSync(path.join(path.dirname(__dirname), '..', 'styles/modeThemes.css'), 'utf8');
    this.test(
...
if (isMainModule) {
  ...
  validator.validateTypographyImplementation()
    .then(results => {
      ...
      fs.writeFileSync(
        path.join(path.dirname(__dirname), 'validation', 'typography-validation-report.json'),
...
export default TypographyValidator;